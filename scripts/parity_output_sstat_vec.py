#!/usr/bin/env python3
"""Parity + microbench harness for DG_SM120_OUTPUT_SSTAT_VEC.

Compares the workspace_output kernel chain output between the scalar
sparse_mla_workspace_output_sstat_kernel and the bf16x4 vec-load variant.
Per-d FMA order is preserved across variants so we expect BIT-EXACT.

Caller controls VEC via env (toggle and rerun the script in fresh
processes — static lambdas cache the env value).
"""
import argparse
import os
import sys
...
def main():
    ...
    vec = os.environ.get("DG_SM120_OUTPUT_SSTAT_VEC", "0")
    fast = os.environ.get("DG_SM120_FAST_SPARSE_MLA", "0")
    print(f"DG_SM120_FAST_SPARSE_MLA={fast} DG_SM120_OUTPUT_SSTAT_VEC={vec}",
          flush=True)
    ...
    save_path = os.environ.get(
        "PARITY_OUT", f"/tmp/parity_output_sstat_vec_VEC{vec}.pt"
    )
    ...
    if os.environ.get("MICROBENCH", "0") == "1":
        ...
        # Decode prod shape: b=8 (max-num-seqs), h=32, topk=128
        for shape in [(1, 32, 128), (4, 32, 128), (8, 32, 128), (16, 32, 128)]:
            ...
            print(f"MICROBENCH VEC={vec} b={b} h={h} topk={k}: "
